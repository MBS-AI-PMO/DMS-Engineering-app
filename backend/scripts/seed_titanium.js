...
async function parseMetalsData() {
    ...
    const aboutRegex = /image:\s+ titaniumAbout/g;
    if (aboutRegex.test(content)) {
        // titaniumAbout is a special case I just added
...
async function seed() {
    console.log('🔧 Seeding TITANIUM (GRADE 5) only...\n');
    ...
    try {
        ...
        // Step 3: Filter for Titanium Grade 5
        const metal = metalsData.find(m => m.name === 'TITANIUM (GRADE 5)');
        if (!metal) {
            throw new Error('TITANIUM (GRADE 5) not found in metalsData.js');
        }
