...
def detect_holes_from_2d_pattern(flat_faces):
    ...
    After unfolding, holes on ALL flanges appear as inner wire loops in the
    flat faces — already in the correct 2D coordinate space.  This catches
    holes that 3D cylinder-based detection misses (e.g. holes on bent flanges).
    """
    holes = []
    seen = []  # (cx, cy, radius) for dedup

    for flat_face in flat_faces:
        try:
            wires = flat_face.Wires
        except Exception:
            continue
        ...
        # Wires[0] = outer boundary; Wires[1:] = inner cutouts (holes)
        for wi in range(1, len(wires)):
            ...
            # --- Method 1: geometric curve type (exact) ---
            is_circle = True
            circle_centers = []
            circle_radii = []
            ...
            for edge in edges:
                try:
                    ct = edge.Curve.TypeId
                    if ct in ("Part::GeomCircle", "Part::GeomArcOfCircle"):
                        c = edge.Curve.Center
                        circle_centers.append(np.array([c.x, c.y]))
                        circle_radii.append(float(edge.Curve.Radius))
                    else:
                        is_circle = False
                        break
                except Exception:
            ...
            if is_circle and circle_centers:
                avg_center = np.mean(circle_centers, axis=0)
                avg_radius = float(np.mean(circle_radii))
            else:
                # --- Method 2: vertex-based circle fit (fallback) ---
                pts = []
                for edge in edges:
                    for v in edge.Vertexes:
                        p = [v.Point.x, v.Point.y]
                        if not pts or (abs(p[0] - pts[-1][0]) > 0.01
                                       or abs(p[1] - pts[-1][1]) > 0.01):
                            pts.append(p)
                ...
                # Circularity check: reject if deviation > 15%
                if np.std(distances) / avg_radius > 0.15:
                    continue

                avg_center = np.array([cx, cy])

            if avg_radius < 0.25:
                continue

            cx, cy = float(avg_center[0]), float(avg_center[1])

            # Dedup (same centre within 2 mm and similar radius)
            is_dup = False
            for sc in seen:
                if (abs(cx - sc[0]) < 2.0
                        and abs(cy - sc[1]) < 2.0
                        and abs(avg_radius - sc[2]) < 0.5):
                    is_dup = True
                    break

            if not is_dup:
                seen.append((cx, cy, avg_radius))
                diam = avg_radius * 2.0
                holes.append({
                    "id": f"hole_{len(holes) + 1}",
                    "diameter_mm": round(diam, 4),
    ...
    # --- Merge counterbores (concentric circles -> keep smallest) ---
    final = []
    merged = set()
    for i, h1 in enumerate(holes):
        if i in merged:
